...
divvy_stations = json.loads(open('/Users/ashutayal/Documents/GitHub/Harris-Challenge-1/divvy_stations.txt').read())
...
for i in range(len(divvy_stations)):
    total_empty_docks += divvy_stations[i]['num_docks_available']

print("Average # of empty docks:", total_empty_docks/len(divvy_stations))

available_bikes = 0
for i in range(len(divvy_stations)):
    available_bikes += divvy_stations[i]['num_bikes_available']

print("Total # of bikes across all docks:", available_bikes)

# PROBLEM 2
# find ratio of bikes that are currently rented to total bikes in the system (ignore ebikes)

# Assuming empty dock means rented bike.
print("The required ratio is:", total_empty_docks/(total_empty_docks+available_bikes))
...
def updater(dict):
    a = dict['num_bikes_available']/(dict['num_bikes_available']+dict['num_docks_available'])
    return "{:.2%}".format(a)

for i in range(len(divvy_stations)):
    divvy_stations[i].update({'percent_bikes_remaining': updater(divvy_stations[i])})